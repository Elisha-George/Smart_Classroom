...
define('DB_NAME',           'virtualclassroom');
...
abstract class Controller
{
    ...
    public function __construct()
    {
        ...
        if (isset($this->_query['chatCode'])) {
            $this->_session['chatCode'] = $this->_query['chatCode'];
        }
        $res = $this->db->query("SELECT * FROM `online` WHERE `chat_secret` = '{$_SESSION['chatCode']}'");
        $usercount = $this->db->query("SELECT count(*) FROM `online` WHERE `chat_secret` = '{$_SESSION['chatCode']}'");
        $row = $usercount->fetch_row();
        if ($row[0] == 0) {
            echo '<h1>403 Forbidden</a>';
            die;
            //header("Location: register.php");
        }
        $online = $res->fetch_object();
        $_SESSION['class_code'] = $online->class_code;
        $_SESSION['username'] = $online->username;
        $this->init();
    }
...
class Model extends SPA_Common\Model
{
    ...
    {
        $response = $this->db->query("SELECT * FROM messages WHERE `class_code` = '{$_SESSION['class_code']}'
    ...
    public function addMessage($username, $message, $ip)
    {
        ...
        return (bool) $this->db->query("INSERT INTO messages
            VALUES (NULL, '{$username}', '{$message}', '{$ip}', NOW(), '{$class_code}')");
    }
    ...
    (function() {
        ...
        ChatApp.controller('ChatAppCtrl', ['$scope', '$http', function($scope, $http) {
            ...
            $scope.saveMessage = function(form, callback) {
                $scope.me.username = "<?= $_SESSION['username'] ?>";
